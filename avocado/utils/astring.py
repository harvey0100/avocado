...
def bitlist_to_string(data):
    ...
    Converts a list of bits (0s and 1s) to an ASCII string representation.
    Only complete bytes (8 bits) are processed; partial bytes are ignored.

    :param data: List of integers (0 or 1) representing bits to be transformed
    :type data: list[int]
...
def string_to_bitlist(data):
    ...
    :type data: str
    :returns: List of integers (0 or 1) representing the bits of each character
    :rtype: list[int]
...
def shell_escape(command):
    ...
    :type command: str
    :returns: The escaped command string (without surrounding quotes)
    :rtype: str

    .. warning::
       The returned string does NOT include the surrounding double quotes.
       The caller must add them when constructing the final command.

    .. rubric:: Escaped Characters
    ...
    * `````  -> ``\\``` (backtick)
    ...
    >>> shell_escape('echo "Hello $USER"')
    'echo \\"Hello \\$USER\\"'
    >>> shell_escape('ls /tmp && echo done')
    'ls /tmp && echo done'

    .. seealso::
       `Bash Manual - Quoting <http://www.tldp.org/LDP/abs/html/escapingsection.html>`_
    """
    escape_chars = {"\\": "\\\\", "$": r"\$", '"': r"\"", "`": r"\`"}
...
def iter_tabular_output(matrix, header=None, strip=False):
    """
    Generator for a pretty, aligned string representation of a matrix.

    Creates a formatted, column-aligned representation suitable for printing
    tabular data such as database results or command output. Dynamically
    determines column widths by scanning all elements.
    ...
    :type strip: bool
    :yields: Formatted string for each row with proper column alignment
    :rtype: Iterator[str]

    .. note::
       Console escape sequences in cell contents are properly handled when
       calculating column widths, ensuring correct alignment even with
       colored text.

    .. rubric:: Column Width Calculation

    * Scans all elements to find the maximum width for each column
    * Handles variable numbers of columns per row
    * Accounts for console codes when measuring text width

    .. rubric:: Example

    >>> matrix = [['Name', 'Age'], ['Alice', '25'], ['Bob', '30']]
    >>> list(iter_tabular_output(matrix[1:], header=matrix[0]))
